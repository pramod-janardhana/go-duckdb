...
func NewDataTransformService() *dataTransform {
	p := path.Join(config.GRPC.DuckDBDir, fmt.Sprintf("data-%s.duckdb", time.Now().String()))
...
func (t dataTransform) Transform(_ *emptypb.Empty, stream pb.DataTransform_TransformServer) error {
	...
	// mem profiler
	{
		p := path.Join(config.WEB_SOCKET.ProfDir, fmt.Sprintf("grpc-mem-%s.prof", time.Now().UTC().Format("2006-01-02 15:04:05")))
		f, err := os.Create(p)
		if err != nil {
	...
	// cpu profiler
	{
		p := path.Join(config.WEB_SOCKET.ProfDir, fmt.Sprintf("grpc-cpu-%s.prof", time.Now().UTC().Format("2006-01-02 15:04:05")))
		f, err := os.Create(p)
		if err != nil {
			fmt.Printf("error creating cpu profiler, err: %v\n", err)
			return err
	...
	limit, offset := config.GRPC.ChunkSize, 0
	sequencyNumber := 1
	for {
